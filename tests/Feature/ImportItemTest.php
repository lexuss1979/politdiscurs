...
class ImportItemTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->data = ['1','II_Title','doc','Topic3Level','Annotation','Ivanov, Sidorov ','SourceName','www.link.com',
            'PostOffice','www.link-to-article.ru','2019','Europe, America','test','',false, false, '','MainTopic','RootTopic',false];
    }
    ...
    public function it_can_store_to_db_and_generate_html()
    {
        ...
        $art = $this->store($this->data, $rootTopic);
        ...
        $this->assertEquals('www.link.com',$art->source->link);

        $this->assertEquals('www.link-to-article.ru',$art->link);
    ...
    public function it_can_store_to_db_and_link_file()
    {
        ContentType::create(['code' => ContentType::ARTICLE, 'name' => 'Статьи']);
        $rootTopic = Topic::create(['title'=>'RootTopic','parent_topic_id'=>null]);
        $this->data[ImportItem::FORMAT_INDEX]= 'pdf';
        $this->data[ImportItem::FILENAME_INDEX] = 'example';
        $expectedFile = config('filesystems.disks.public.files').'/example.pdf';

        $art = $this->store($this->data, $rootTopic);
        $this->assertNotNull($art->file);
        $this->assertEquals('example.pdf',$art->file->filename);
    ...
    /** @test */
    public function it_can_detect_item_format_type()
    {
        $this->data[ImportItem::FORMAT_INDEX]= 'pdf';
        $ii = new ImportItem($this->data);
        $this->assertEquals(Article::PDF_TYPE,$ii->format());

        $this->data[ImportItem::FORMAT_INDEX]= 'link';
        $ii = new ImportItem($this->data);
        $this->assertEquals(Article::LINK_TYPE, $ii->format());

        $this->data[ImportItem::FORMAT_INDEX]= 'doc';
        $ii = new ImportItem($this->data);
        $this->assertEquals(Article::TEXT_TYPE,$ii->format());

    }

    /** @test */
    public function it_throw_exception_if_item_format_type_unrecognized()
    {
        $this->data[ImportItem::FORMAT_INDEX]= 'word';
        $ii = new ImportItem($this->data);
